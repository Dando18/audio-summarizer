...
class OpenAISummarizer(Summarizer):
    ...
    def init_api_key(self, api_key: Optional[str] = None):
        if not api_key:
            ...
            if not openai.api_key:
                raise ValueError("No OpenAI API key found. Set OPENAI_API_KEY environment variable or pass api_key " +
                                 "to constructor.")
        else:
    ...
    # overrides
    def summarize(self, text: str, max_tokens: int = 999, temperature: float = 0.7) -> str:
        model = self.get_model(text, response_tokens=max_tokens)
        if model in ["text-davinci-003"]:
            completion = openai.Completion.create(model=model, prompt=text, temperature=temperature)
